...
precision=0; //espace entre interieur enclosure du haut et exterieur enclosure du bas.  (1 = .5 de chaque cote)
...
translate([2*longueur+5,0,0])
    difference(){
        difference(){
            ...
            translate([0.5*longueur,(largeur-ou)/2,(hauteur+epbas+ephaut)/2-ou/2]) cube(longueur,ou,ou+hauteur,0);
        }
        translate([-longueur/2,+4,1])rotate([180,0,0])linear_extrude(3,center=true) text(prenom,10);
    }
...
translate([21,0,0])
    ...
    translate([-longueur/2+2,4,1])rotate([180,0,0])linear_extrude(3,center=true) text(prenom,10);
}
